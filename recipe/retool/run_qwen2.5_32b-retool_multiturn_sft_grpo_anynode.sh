...
WANDB_EXPERIMENT_BASE=qwen2.5-32b_function_rm-retool-async-sgl-sft-grpo-mb-bsz${train_prompt_bsz}-n${n_resp_per_prompt}-${RESPONSE_LENGTH}-${MAX_ASSISTANT_TURNS}turns
...
if [ $RANK -eq 0 ]; then
    python3 -m verl.trainer.main_ppo \
        ...
        actor_rollout_ref.model.path=$HOME/checkpoints/retool-multiturn-sft/retool-multiturn-sft-qwen2.5-32b-sp8-new-template-bf16/global_step_84 \
        ...
        actor_rollout_ref.rollout.multi_turn.tool_config_path="$PROJECT_DIR/recipe/retool/config/tool_config/sandbox_fusion_retool_config.yaml" \
