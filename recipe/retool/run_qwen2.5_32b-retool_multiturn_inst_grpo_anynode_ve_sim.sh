...
WANDB_EXPERIMENT_BASE=qwen2.5-32b_function_rm-retool-async-sgl-inst-grpo-ve-sim-bsz${train_prompt_bsz}-n${n_resp_per_prompt}-${RESPONSE_LENGTH}-${MAX_ASSISTANT_TURNS}turns
...
data.train_files=$HOME/dataset/retool_dapo/train.parquet \
data.val_files=$HOME/dataset/retool_aime2024/train.parquet \
actor_rollout_ref.model.path=$HOME/model/Qwen2.5-32B-instruct \
...
actor_rollout_ref.rollout.multi_turn.tool_config_path=$PROJECT_DIR/recipe/retool/config/tool_config/sandbox_fusion_retool_config_sim_jupyter_vefaas.yaml \
