...
class RayPPOTrainer:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        # Process rollout metrics using standardized aggregation
                        metrics.update(self._process_rollout_metrics(gen_batch_output))
    ...
    def _process_rollout_metrics(self, gen_batch_output):
        ...
        try:
            ...
            logger.debug(
                f"Processing metrics: tool={len(tool_metrics_data)}, conversation={len(conversation_metrics_data)}"
            )
            ...
            if tool_metrics_data or conversation_metrics_data:
                ...
                try:
                    batch_metrics = aggregate_batch_metrics(
                        tool_metrics_list=tool_metrics_data if tool_metrics_data else None,
                        conversation_metrics_list=conversation_metrics_data if conversation_metrics_data else None,
                ...
                if batch_metrics.tool_metrics:
                    try:
                        ...
                        if tool_metrics_data:
                            ...
                            flattened_categorical = MetricsAggregator.flatten_categorical_metrics(
                                categorical_results, "tools/"
                            )
                            metrics_to_log.update(flattened_categorical)

                        logger.debug(
                            "Processed tool metrics: auto-detected scalars + stats expansion + "
                            f"{len(flattened_categorical) if tool_metrics_data else 0} categorical metrics"
                        )
                    except Exception as e:
                ...
                if batch_metrics.conversation_metrics:
                    try:
                        ...
                        if conversation_metrics_data:
                            categorical_conv_results = MetricsAggregator.aggregate_categorical_fields(
                                conversation_metrics_data
                            )
                            flattened_conv_categorical = MetricsAggregator.flatten_categorical_metrics(
                                categorical_conv_results, "conversations/"
                            )
                            metrics_to_log.update(flattened_conv_categorical)

                        logger.debug(
                            "Processed conversation metrics: auto-detected scalars + stats expansion + "
                            f"{len(flattened_conv_categorical) if conversation_metrics_data else 0} categorical metrics"
                        )
                    except Exception as e:
        ...
        logger.debug(
            f"Total processed metrics: {len(metrics_to_log)} in {(time.time() - processing_start_time) * 1000:.2f}ms"
        )
        return metrics_to_log
