...
class RayPPOTrainer:
    ...
    def _create_dataloader(self):
        ...
        from verl.utils.import_utils import load_extern_type # From main side import
        ...
        if "custom_cls" in self.config.data and self.config.data.custom_cls.get("path", None) is not None:
            ...
            tool_config_path = OmegaConf.select(self.config, "actor_rollout_ref.rollout.multi_turn.tool_config_path")
            need_tools_kwargs = tool_config_path is not None
            self.train_dataset = dataset_cls(parquet_files=self.config.data.train_files,
                                            tokenizer=self.tokenizer,
                                            processor=self.processor,
