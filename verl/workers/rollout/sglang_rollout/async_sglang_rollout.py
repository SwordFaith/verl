...
class AsyncSGLangRollout(BaseRollout):
        ...
        if config.multi_turn.tool_config_path is not None:
            ...
            def initialize_tools(tool_config) -> List:
                ...
                for tool_config in tool_config.tools:
                    ...
                    tool = tool_cls(
                        config=OmegaConf.to_container(tool_config.config, resolve=True), tool_schema=tool_schema
                    )
    ...
    async def _async_rollout_a_request(
        ...
        tool_reward_tasks = [calc_reward_and_release_fn(name, tool) for name, tool in self._tool_map.items()]
