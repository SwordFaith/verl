...
def test_sglang_rollout():
    ...
    actor_model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=dtype, device_map="cuda")
    ...
    rollout = AsyncSGLangRollout(
        actor_module=model_path, config=rollout_config, tokenizer=tokenizer, model_hf_config=actor_model.config
    )
